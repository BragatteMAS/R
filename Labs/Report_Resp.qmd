...
## Função para pivotar e somar os resultados por faixa etária e patógeno
pivot_and_summarize <- function(data, pathogen, time_unit) {
  data %>%
    filter(get(paste0(pathogen, "_test_result")) %in% c("Pos", "Neg")) %>%
    group_by_at(vars(time_unit, paste0(pathogen, "_test_result"), "age_group")) %>%
    ...
    mutate(pathogen = pathogen, country = "Brasil", .before = 1)
}

## Aplicar a função para cada patógeno e juntar os resultados para epiweek
result_epiweek <- lapply(pathogens, function(p) pivot_and_summarize(combined, p, "epiweek")) %>%
  bind_rows()

## Aplicar a função para cada patógeno e juntar os resultados para month
result_month <- lapply(pathogens, function(p) pivot_and_summarize(combined, p, "month")) %>%
  bind_rows()
...
print(paste("Total de análises da última semana:", total_ultima_semana))
print(paste("Total cumulativo do último ano:", total_cumulativo_ultimo_ano))
```
