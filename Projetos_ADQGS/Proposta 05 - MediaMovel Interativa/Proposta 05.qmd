...
> Como informar a população durante a pandemia sobre situação de casos?
...
covid_br <-
    ...
    filter.(location == "Brazil",
            between.(date, as.Date('2022-01-01'),
                    as.Date('2022-11-19'))) |>
    ## columas de média móvel 7 e 15 dias para criar
...
#### Editando planilha no código

```{r}
## Salvando alterações no arquivo existente em .R
data_edit (gov_br_estados, code = TRUE)
```
...
gov_br_est <- 
    ...
    gov_br_estados |> 
    ## quais colunas eu quero
    select.(location, epi_week, date, new_Cases ,new_Deaths)
...
casos_br <- ggplot(covid_br) +
    ## mapa
    aes(x = date, y = m.movel.7_cases, fill = location) +
    ## geometria
    geom_col() +
    ## formatações
...
Transformando em um gráfico interativo
