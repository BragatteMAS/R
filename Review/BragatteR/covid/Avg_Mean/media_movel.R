...
casos <- ggplot() +
    ...
    geom_line(data = dados_brasil, aes(x=date, y=ma7_confirmed),size = 1.82, colour = "#ef562d") +
...
filter <- dados_brasil %>%
    filter(date >= "2020-02-25" & date <= "2020-10-20")
casos31 <- ggplot() + 
    geom_bar(data = filter, aes(x=date, y=new_confirmed), stat = 'identity', fill = "#0c4c8a") +
    geom_line(data = filter, aes(x=date, y=ma7_confirmed),size = 1.82, colour = "#ef562d") +
...
#library(lemon)
#library(cowplot)
# get legend if not Null
#leg <- get_legend(final)
#turno legend into plot
#lega <- as_ggplot(leg)
#Grid plotgrid
#final_l <- grid_arrange_shared_legend(casos,casos31, obitos,obitos31, nrow = 2, ncol = 2)
#final_l